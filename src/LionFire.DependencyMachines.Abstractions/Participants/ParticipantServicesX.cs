...
public static class ParticipantConfigX
{
    //public static DependencyMachineConfig AddParticipant<T>(this DependencyMachineConfig config, Func<T, Task> configure, params object[] constructorParameters)
    //    where T : IParticipant
    //{
    //    throw new NotImplementedException();
    //}
    public static DependencyMachineConfig AddParticipant<T>(this DependencyMachineConfig config, Action<T> configure, params object[] constructorParameters)
        where T : IParticipant
    {
        config.InjectedParticipants.Add(serviceProvider =>
        {
    ...
    public static DependencyMachineConfig AddParticipant<T>(this DependencyMachineConfig config, string participantName, Action<T> configure, params object[] constructorParameters)
          where T : IParticipant
    {
        config.InjectedParticipants.Add(serviceProvider =>
        {
...
public static class ParticipantServicesX
{
    ...
    where T : IParticipant
    {
        services.Configure<DependencyMachineConfig>(c => c.AddParticipant<T>(configure, constructorParameters));
    ...
    public static IServiceCollection AddParticipant<T>(this IServiceCollection services, string participantName, Action<T> configure, params object[] constructorParameters)
          where T : IParticipant
    {
        services.Configure<DependencyMachineConfig>(c => c.AddParticipant<T>(participantName, configure, constructorParameters));
        return services;
    }


#if UNUSED
    // Named DependencyMachine, Named Participant (TODO: .NET 8 Keyed Service Collection)
