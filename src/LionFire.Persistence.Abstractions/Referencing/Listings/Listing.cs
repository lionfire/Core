        ...
        if (typeof(TValue).IsGenericType)
        {
            ...
            if (genericType == typeof(Metadata<>))
            {
                ...
                if (metadataType.IsGenericType && metadataType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
                {
                    ...
                    if (enumerableType.IsGenericType && (enumerableType.GetGenericTypeDefinition() == typeof(IListing<>) || enumerableType.GetGenericTypeDefinition() == typeof(Listing<>)))
                    {
...
public interface IListing<out T>
{
    string Name { get; }
...
// REVIEW: Consider replacing IListing<T> with Listing, and doing type filtering another way
public class Listing<T> : IListing<T>
{
    ...
    public static IListing<T> CreateDirectoryListing(string name) => new Listing<T>(name, type: null, directory: true);
    ...
    //    IsDirectory = directory;
    //}
    public static implicit operator Listing<T>(string name) => new Listing<T> ( name );
