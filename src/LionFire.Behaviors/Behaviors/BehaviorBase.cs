...
#endif
{
    ...
    public string Name { get; set; } // REVIEW - recently uncommented in order to get things compiling
    ...
    /// OBSOLETE this -- see WaitForCondition's Succeed/Fail when passing/failing
    /// </summary>
    ...
    protected BehaviorStatus FaultOnMissing(string missingItemName)
    {
        return Fault("Missing dependency: " + missingItemName);
    }
    protected BehaviorStatus FaultOnMissing(Type type)
    {
        return Fault("Missing dependency of type " + type.Name);
    }
    protected BehaviorStatus FaultOnMissing<type>()
    {
        return Fault("Missing dependency of type " + typeof(type).Name);
    }
    protected BehaviorStatus FailOnMissing<type>()
    {
        return Fail("Requires a " + typeof(type).Name);
    }

    protected BehaviorStatus Fault(string msg = null)
    {
    ...
    protected BehaviorStatus Fail(string msg = null)
    {
        switch (Status)
        {
...
#else
    private static ILogger l = NullLogger.Instance;
#endif
