...
//, Dispatcher dispatcher
        ...
        private void OnCollectionChanged(object sender, NotifyCollectionChangedEventArgs eventArgs)
        {
            ...
            catch (Exception ex)
            {
                if (firstTime)
                {
                    Trace.WriteLine("[warn] Exception when raising event.  Trying again with reset event.  Exception: " + ex.ToString());
                ...
                {
                    Trace.WriteLine("[error] Exception when raising event. Exception: " + ex.ToString());
                }
            }
        }
