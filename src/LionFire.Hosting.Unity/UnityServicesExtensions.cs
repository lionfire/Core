using LionFire.Services;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using System;
using System.Collections.Generic;
using System.Text;
using UnityEngine;

namespace LionFire.Hosting.Unity
{
    public static class UnityServicesExtensions
    {
        public static IHostBuilder AddUnityRuntime(this IHostBuilder hostBuilder, MonoBehaviour monoBehaviour)
            => hostBuilder.ConfigureServices(services => services.AddUnityRuntime(monoBehaviour));

        public static IServiceCollection AddUnityRuntime(this IServiceCollection services, MonoBehaviour monoBehaviour)
            => services
                .AddUnityEngine()
                .AddHostedService<UnityStartStopLogger>()
                .If(monoBehaviour != null, s => s.AddSingleton<MonoBehaviour>(monoBehaviour)) // Useful for registering coroutines/repeating methods?
                ;
    }
}
