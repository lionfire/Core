    ...
    public class EnableableBase : IEnableableEx, IPropertyChanged, INotifyPropertyChanged
    {
        ...
        public bool Enabled
        {
            ...
            set
            {
                ...
                {
                    UpdateCanEnable(true);
                    if (!CanEnable) throw new Exception("CanEnable is false: " + CannotEnableMessage);
                }
        ...
        public string CannotEnableMessage
        {
            get
            {
                ...
                return CannotEnableReasons.ToArray().Any() ? CannotEnableReasons.ToArray().Select(r => r.Message).Aggregate((x, y) => x.ToString() + Environment.NewLine + y.ToString()) : "";
