...
{
    public class ExecutionTypeResolver
    {

        public List<IExecutionConfigResolver> DefaultSourceUriResolvers  = new List<IExecutionConfigResolver>{
        ...
        public async Task<bool> ResolveSourceUri(ExecutionConfig c)
        {
bool result = false;
            //if (c.SourceUriScheme != null)
            {
                switch (c.SourceUriScheme)
                {
                    ...
                    case "https":
                        ...
                        return true;
                    case "gist":
                        throw new NotImplementedException("gist support coming soon");
                        break;
                    case "github":
...
c.Object = null;
            ...
            bool result = await ResolveSourceUri(c);
...
Console.WriteLine(ex.ToString());
  ...
  /*      public async Task<bool> ResolveAssembly(ExecutionConfig c)
    ...
    public static class ExecutionTypeResolverExtensions
    {
        ...
        {
            return await Singleton<ExecutionTypeResolver>.Instance.ResolveObject(config);
        }
        public static async Task<bool> ResolveType(this ExecutionConfig config)
        {
            return await Singleton<ExecutionTypeResolver>.Instance.ResolveType(config);
        }
