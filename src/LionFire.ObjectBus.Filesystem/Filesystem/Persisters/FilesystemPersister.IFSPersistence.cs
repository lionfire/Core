...
namespace LionFire.Persistence.Filesystem
{
    ...
    public abstract partial class FilesystemPersister<TReference, TPersistenceOptions> : IFilesystemPersistence<TReference, TPersistenceOptions>
    {
        #region IO
        ...
        {
            if (PersistenceOptions.MaxDeleteRetries == 0)
            {
            ...
            else
            {
                return ((Func<Task<bool?>>)(() => DeleteFileOrDirectory(fsPath)))
                    .AutoRetry(maxRetries: PersistenceOptions.MaxDeleteRetries, millisecondsBetweenAttempts: PersistenceOptions.MillisecondsBetweenDeleteRetries);
            }
        }
