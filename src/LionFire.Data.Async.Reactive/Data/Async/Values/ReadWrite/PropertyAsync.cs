...
[Obsolete("Use AsyncValueRx<TKey,TValue>?")] // OLD
public class PropertyAsync<TObject, TValue>
    : ReadPropertyAsync<TObject, TValue>
{
    #region Lifecycle

    public PropertyAsync(TObject target, AsyncValueOptions? options = null) : base(target, options)
    {
    }
    ...
    public override ITask<IGetResult<TValue>> Get(CancellationToken cancellationToken = default)
    {
        ...
        if (IsSetting && Options.OptimisticGetWhileSetting)
        {
