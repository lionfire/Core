...
namespace LionFire.Avalon
{
    ...
    public class FluidWrapPanel : Panel
    {
        ...
        {
            FluidWrapPanel panel = (FluidWrapPanel)d;
            IEnumerable oldItemsSource = 
                //(ObservableCollection<UIElement>)
               (IEnumerable)e.OldValue;
        ...
        public static readonly DependencyProperty AllowReorderProperty =
            DependencyProperty.Register("AllowReorder", typeof(bool), typeof(FluidWrapPanel),
                new FrameworkPropertyMetadata((bool)true
                    ));

        /// <summary>
        /// Gets or sets the AllowReorder property. This dependency property 
        ...
        private void InitializeArrange()
        {
            foreach (UIElement child in fluidElements)
            {
                ...
                if (index == -1)
                {
                    throw new Exception("Internal exception.  Failed to find element in this panel's collection of items.");
                    //continue;
                }
        ...
        private bool UpdateDragElementIndex(int newIndex)
        {
            ...
            if (dragCellIndex == -1)
            {
                Visual visual = dragElement as Visual;
                if (visual != null)
                {
                    var visualParent = VisualTreeHelper.GetParent(dragElement);
                    dragCellIndex = fluidElements.IndexOf(visualParent as UIElement);
                }
                if (dragCellIndex == -1)
                {
                    throw new Exception("Internal exception.  Failed to find element in this panel's collection of items.");
