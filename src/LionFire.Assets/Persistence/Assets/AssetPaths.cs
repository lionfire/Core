...
{
    // REVIEW - This was imported -- does it need to be ported/redesigned?  Review what is Unused
    ...
    public static class AssetPaths
    {
        ...
        private static object assetPathsLock = new object();

        #region Path

        public static string AssetPathFromAssetTypePath<AssetType>(this string subpath)
        {
            return LionPath.Combine(AssetPaths.GetAssetTypeFolder(typeof(AssetType)), subpath);
        }
        ...
        [AotReplacement]
        public static string AssetPathFromAssetTypePath(string assetTypePath, Type type) 
            => LionPath.Combine(AssetPaths.GetAssetTypeFolder(type), assetTypePath);
        public static Func<Type, string> GetTypeKey { get; set; } = t => t.Name;
        ...
        public static string AssetPathFromAssetTypePath(this IAsset asset, string assetTypePath) 
            => LionPath.Combine(AssetPaths.GetAssetTypeFolder(asset), assetTypePath);
        ...
        public static string GetAssetTypeFolder(Type type, bool alwaysAppendLastBase = false)
        {
            ...
            {
                string defaultPath;
                if (assetPaths.TryGetValue(type, out defaultPath))
                {
                    return defaultPath;
                }
                ...
                for (int index = 0; index < types.Count; index++)
                {
                    ...
                    if (usedTypes.Contains(types[index])) continue;
                    ...
                    if (!gotConcreteType /*&& alwaysAppendLastBase*/ && !typeItem.IsAbstract && appendToPath)
                    {
                        ...
                        #region Strip prefix naming convention from ITemplates, ITemplateParameters

                        if (typeName.Length > 1 
                            //&& (typeof(ITemplateAsset).IsAssignableFrom(typeItem) || typeof(ITemplateParameters).IsAssignableFrom(typeItem)) // TOPORT - this line
                            )
                        {
                            char[] prefixChars = new char[] { 'T', 'P' };
