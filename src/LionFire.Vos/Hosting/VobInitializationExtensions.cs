...
namespace LionFire.Services
{
    public static class VobInitializationExtensions
    {
        ...
        public static IServiceCollection InitializeRootVob2(this IServiceCollection services, Action<IVob> action)
        {
            services.Configure<List<VobInitializer>>(list => list.Add(new VobInitializer(action)));
            return services;
        }
        public static IServiceCollection InitializeVob2(this IServiceCollection services, string vobPath, Action<IVob> action)
        {
            services.Configure<List<VobInitializer>>(list => list.Add(new VobInitializer(action) { VobPath = vobPath }));
            return services;
        }
        public static IServiceCollection InitializeVob2(this IServiceCollection services, string vobRootName, string vobPath, Action<IVob> action)
        {
