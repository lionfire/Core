...
public static class IVobInternalsVobNodeExtensions
{
    ...
    public static VobNode<TImplementation> GetOrAddNextVobNode<TImplementation>(this IVob vob, Func<IVobNode, TImplementation> factory = null, bool addAtRoot = false)
        where TImplementation : class
    {
        var vobI = vob as IVobInternals;
        var next = vob.TryGetNextVobNode<TImplementation>();
        if (next != null) return next;

        return (addAtRoot ? vob.Root as IVobInternals : vobI).AcquireOrAddOwnVobNode<TImplementation>(factory);
    }
