...
public class _Resolve
{
    ...
    public void _Resolve_InPlace() => RunTest(async sp =>
    {
        ...
        try
        {
            Assert.AreEqual(10, (long)metrics["LionFire.Persistence.Handles.WeakHandleRegistry.ReadHandlesCreated"].value!);
            Assert.AreEqual(3, (long)metrics["LionFire.Vos.Retrieve"].value!);
            Assert.AreEqual(2, (long)metrics["LionFire.Vos.Retrieve.RetryAfterMountsChanged"].value!);
            Assert.AreEqual(3, (long)metrics["LionFire.Vos.Retrieve.Batch"].value!);
            ...
            Assert.AreEqual(1, (long)metrics["LionFire.Persistence.Filesystem.DirectoryExists"].value!);
